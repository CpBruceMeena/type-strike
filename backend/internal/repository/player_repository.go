...
func (r *PlayerRepository) Create(ctx context.Context, req models.CreatePlayerRequest) (*models.Player, error) {
	...
	err := r.pool.QueryRow(ctx,
		`INSERT INTO players (level, title, xp, total_stars)
		 VALUES ($1, $2, 0, 0)
		 RETURNING id, player_uuid, level, title, xp, total_stars, created_at, last_played_at`,
		level, title,
...
func (r *PlayerRepository) Update(ctx context.Context, id int, req models.UpdatePlayerRequest) (*models.Player, error) {
	...
	err := r.pool.QueryRow(ctx, query, args...).Scan(
		&p.ID, &p.PlayerUUID, &p.Level, &p.Title, &p.XP, &p.TotalStars, &p.CreatedAt, &p.LastPlayedAt,
	)
	if err != nil {
...
func (r *PlayerRepository) AddXP(ctx context.Context, playerID, xp int) (*models.Player, bool, error) {
	...
	for {
		...
		if newXP >= xpRequired && p.Level < 100 {
			newXP -= xpRequired
			p.Level++
			leveledUp = true
		} else {
